...
class Solver(object):
    ...
    def log_losses(self, losses, iteration=None):
        # Existing logging
        logger.info(f"carrier loss: {losses['carrier_loss']}")
        logger.info(f"message loss: {losses['avg_msg_loss']}")
        logger.info(f"carrier SnR: {losses.get('carrier_snr', 'N/A')}")
        logger.info(f"message SnR: {losses.get('msg_snr', 'N/A')}")
        
        # New accuracy logging
        logger.info(f"carrier accuracy: {losses.get('carrier_accuracy', 'N/A')}")
        logger.info(f"message accuracy: {losses.get('msg_accuracy', 'N/A')}")

        # Log to experiment if applicable
        if hasattr(self, 'experiment'):
            self.experiment.log_metric({'carrier_loss': losses['carrier_loss']}, step=iteration)
            self.experiment.log_metric({'message_loss': losses['avg_msg_loss']}, step=iteration)
            self.experiment.log_metric({'carrier_accuracy': losses.get('carrier_accuracy', 0)}, step=iteration)
            self.experiment.log_metric({'message_accuracy': losses.get('msg_accuracy', 0)}, step=iteration)
